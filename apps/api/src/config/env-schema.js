...
exports.EnvironmentValidator = exports.ENV_SCHEMA = void 0;
/**
...
class EnvironmentValidator {
    ...
    static validate(env) {
        const errors = [];
        const warnings = [];
        Object.entries(exports.ENV_SCHEMA).forEach(([envKey, definition]) => {
            const value = process.env[envKey];
            const isRequired = definition.required || definition.requiredIn?.includes(env);
     ...
     */
    static validateType(value, type, key) {
        switch (type) {
            case 'number':
    ...
    static validateProduction() {
        ...
        if (corsOrigin === '*') {
            warnings.push({
                key: 'CORS_ORIGIN',
                warning: 'CORS_ORIGIN is set to * (wildcard) - restrict to specific domains in production',
                severity: 'warning',
                environment: 'production',
            });
        }
        ...
        if (process.env.LOG_LEVEL === 'debug' ||
            ...
            warnings.push({
                key: 'LOG_LEVEL',
                warning: 'LOG_LEVEL is set to debug/verbose - should be info/warn in production',
                severity: 'warning',
                environment: 'production',
            });
        }
    ...
    static generateReport() {
        const schemaVars = Object.entries(exports.ENV_SCHEMA).map(([key, def]) => ({
            name: key,
            ...def,
