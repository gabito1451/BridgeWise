...
exports.VersionDisplay = void 0;
const react_1 = __importDefault(require("react"));
const react_i18next_1 = require("react-i18next");
const useVersion_1 = require("../hooks/useVersion");
/**
 ...
 */
const VersionDisplay = ({ className = '', showDetails = false, enableLogging = true, apiUrl, onClick, }) => {
    const { t } = (0, react_i18next_1.useTranslation)();
    const { version, loading, error } = (0, useVersion_1.useVersion)({
        apiUrl,
        enableLogging,
    });
    const handleClick = () => {
    ...
    if (loading) {
        return (<div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 ${className}`}>
    ...
    if (error) {
        return (<div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 ${className}`}>
        v?.?.?
      </div>);
    }
    if (!version) {
    ...
    return (<div className={`inline-block ${className}`}>
      {/* Simple Badge */}
      {!showDetails ? (<button onClick={handleClick} className={`
        ...
        <div className={`
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded text-xs font-medium ${getVersionColor()}`}>
                v{version.version}
              </span>
            </div>
...
exports.default = exports.VersionDisplay;
