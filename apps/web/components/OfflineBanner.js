...
function OfflineBanner() {
    ...
    const { isOffline, cache } = (0, useOfflineDetection_1.useOfflineDetection)();
    if (!isOffline)
        return null;
    const cachedAt = cache?.cachedAt
        ? new Date(cache.cachedAt).toLocaleTimeString()
        : null;
    return (<div role="alert" aria-live="assertive" className="fixed top-0 inset-x-0 z-50 flex items-center justify-center gap-3 bg-yellow-500 px-4 py-2 text-sm font-medium text-yellow-900 shadow-md">
